...
export function AddItemForm({ onCloseClick, param }) {
    ...
    async function loadItem() {
        const result = await fetch(`${backend}/api/item/${param.itemId}`);
    ...
    return (
        <form className="form" onSubmit={save} id="addItemForm" method="POST" encType="multipart/form-data">
            ...
            <input type="text" className="item-quantity" placeholder="Заказ от" name="quantity" required defaultValue={item?.qantity} />
            ...
            <label >Отображать на главной <input type="checkbox" name="show" checked={item?.show}></input></label>
